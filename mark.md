...
| `code`         | 是       | 发送到代码栏的初始代码                                       |
| `judgeMode`    | 是       | 验证方式：`runSuccess`（运行成功即可）、`testCases` （需要通过测试用例）或 `outputMatch`（运行结果与作者预设答案一致） |
| `code` | 是 | 发送到代码栏的初始代码 |
| `judegeTests` | 条件必填 | `judgeMode: testCases` 时必填，表示实际执行的测试用例 |
| `judegeOutput` | 条件必填 | `judgeMode: outputMatch` 时必填，表示作者规定的完整输出 |
...
平台先执行用户代码，再自动执行作者写在 `tests` 中的测试用例。所有测试通过才判定正确。
...
平台执行用户完成后的整段代码，捕获代码产生的全部标准输出，再与 `expectedOutput` 比较。两者一致时判定正确。
...
`expectedOutput` 可以包含多行内容。平台会将程序的完整输出与其进行整体比较：
